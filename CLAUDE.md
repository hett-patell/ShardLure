...
- **Themes: only 3 are implemented** (`dragon`, `meridian`, `sprite`) — `themes.css` defines exactly those and `validateSetting` hard-rejects anything else. **The README's "Noctiluca" (4th theme) does not exist in code.** Live switching sets `<html data-theme>`, persists `ui.theme` to the keystore (server > localStorage, synced across tabs via `BroadcastChannel`), and swaps the globe engine (Dragon→globe.gl, Meridian/Sprite→Cobe).
- **`demos/themes/` is NOT the live UI** — those (`phosphor`, `anode`, `linear`, `ledger`, `clearing`, `vault`, `receipt`) are standalone design *studies*, not embedded and not routed. Treat as exploration only.
