...
- Assets are `go:embed`ed (`embed.go`): `index.html` + `intel.html` (two dashboards), `themes.css`, `cobe-globe.js`/`cobe-boot.js`, `vendor/vis-network.min.js`, `vendor/cobe.esm.js` (vendored globe engine — no CDN; script-src CSP is 'self'-only), and `stickers/*.svg`. HTML is served behind auth; static JS/CSS/stickers are unauthenticated.
