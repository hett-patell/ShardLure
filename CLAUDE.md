...
**The one-key invariant.** abuse.ch issues a single Auth-Key per account covering MalwareBazaar *and* URLhaus. There is exactly one resolution path on each side — `web.abuseCHKeyLive()` (with `bazaarKeyLive()` as a thin alias) and `cmd.abuseCHKey()` — both DB > env > config. Deliberately **no `urlhaus` key setting exists** in the keystore registry: a second field would let the two silently drift and half-arm sharing. The CLI reads the keystore FIRST because a key saved from the Settings panel lives only in `app_settings` (config is seeded *into* the keystore at startup, never the reverse) — a config-only lookup reported "no key" on exactly the deployments that had one.
