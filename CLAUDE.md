...
Seeds history, then runs **four background goroutines** + the web server (main goroutine), all sharing one `*store.Store` and one `*settings.Keystore`, shut down via `signal.NotifyContext`:
...
- **Migrations** are a linear ladder v1→v17 in `sqlite.go`, each guarded `if current < N` and stamped `INSERT OR IGNORE` into `schema_migrations`. A base `CREATE TABLE IF NOT EXISTS` block runs first (fresh DBs get everything, ladder is near-no-op); old DBs get `ALTER TABLE ADD COLUMN` backfills. **Most migrations are index tuning driven by EXPLAIN QUERY PLAN** — read the comments before touching them (e.g. v9 *drops* a write-amplifying index; v10 replaces `idx_events_actor` with composite `(actor_id, ts)`). The tail is feature schema: v16 adds the `cowrie_session_{hassh,meta}` side-channel tables, v17 adds artifact retry state (`attempt_count`, `next_attempt_at`) and is the one migration that has to `CREATE TABLE IF NOT EXISTS artifacts` itself, because that table is otherwise lazily created and the migration needs it now. To add schema: append the next version to the ladder, don't renumber — **and bump the "v1→v17" figure above**, which has silently drifted twice.
...
### Outbound intel (`internal/intel/{bazaar,urlhaus,abuseipdb,ioc}`) — the vetting-gate pattern
...
**The one-key invariant.** abuse.ch issues a single Auth-Key per account covering MalwareBazaar *and* URLhaus. There is exactly one resolution path on each side — `web.abuseCHKeyLive()` (with `bazaarKeyLive()` as a thin alias) and `cmd.abuseCHKey()` — both DB > env > config. Deliberately **no `urlhaus` key setting exists** in the keystore registry: a second field would let the two silently drift and half-arm sharing.
...
- Assets are `go:embed`ed (`embed.go`): `index.html` + `intel.html` (two dashboards), `themes.css`, `cobe-globe.js`/`cobe-boot.js`, `vendor/vis-network.min.js`, `vendor/cobe.esm.js` (vendored globe engine — no CDN; script-src CSP is 'self'-only), and the brand assets `logo.svg`/`logo-180.png`/`favicon.ico`. HTML is served behind auth; static JS/CSS and the brand assets are unauthenticated — a favicon is requested before any token exists, and none of them carry telemetry.
